...
本システムは、自律的な動作を実現するために `--dangerously-skip-permissions` (Claude Code) および `--yolo` (Gemini CLI) フラグを使用しています。これは、AIエージェントが**ユーザーの確認なしに**システムコマンドの実行やファイルの変更を行うことを許可するものです。

- **潜在的なリスク**: 誤ったファイル削除、有害なコマンドの実行、信頼できないコンテンツを処理する際のプロンプトインジェクション攻撃など。
- **推奨事項**: 機密データや重要なシステムファイルが含まれていない、隔離された環境（サンドボックス化されたWSL2、仮想マシン、Dockerコンテナなど）で実行することを強く推奨します。
- **免責事項**: 本ソフトウェアの使用によって生じた損害やデータの損失について、開発者は一切の責任を負いません。
